...
These are informational only. Do not act on them.
...
1. **Task Summary** — which task was implemented, one-sentence description.
2. **TDD Log** — a brief summary of each test-then-implement cycle (behavior targeted, test written, implementation approach).
3. **Files Changed** — list of files created or modified, with a one-line description of each change.
4. **Test Results** — pass/fail summary for the task's tests and the broader test suite.
5. **Discovered Issues** — any out-of-scope issues noted during implementation (or "None").
6. **OpenSpec Update Confirmation** — confirmation that `tasks.md` was updated.
7. **Next Step** — prompt the user to select the next task or proceed to QA review.
