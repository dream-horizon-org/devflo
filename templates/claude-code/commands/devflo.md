---
description: Run the complete AI SDLC pipeline from classification through delivery
...
Invoke the **PM Agent** (`pm-agent` agent) with:
...
Invoke the **Architect Agent** (`architect-agent` agent) with:
...
2. Invoke the **Developer Agent** (`dev-agent` agent) with:
   - The change name and workspace path
...
Invoke the **QA Agent** (`qa-agent` agent) with:
