...
4. If the feedback is critically ambiguous (could lead to contradictory outcomes), you may ask **at most one** clarifying question. Otherwise, do not ask questions.
...
- **ALWAYS** present structured, option-based questions. Never ask open-ended questions.
- For New Feature and Major Refactor: you almost certainly have questions. If you find yourself producing a brief without having asked questions at all, **stop and reconsider** — you are likely over-inferring.
- Every question must have 2–5 concrete options with trade-off descriptions, plus a "You decide — pick the best option" choice.
- Batch independent questions together (max 5 per batch). Ask dependent questions sequentially.
...
If **2+ dimensions are "No"** → request is underspecified → you MUST ask clarifying questions before producing the brief.
...
- **Critical unknowns**: affects scope, UX, business rules, or acceptance criteria → **MUST ask user**. Never infer. When in doubt, classify as critical.

### Step 4 — Ask Questions

Present questions to the user as numbered options for every critical unknown. Rules:
...
If scope includes 3+ distinct capabilities/endpoints, present a scope reduction question to the user with options: (a) keep full scope, (b) defer specific items. Depth over breadth.
...
- **Default posture: Ask.** Vague request → present numbered options to the user. Never guess.
